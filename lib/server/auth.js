...
exports.routes = {
    ...
    post:{
        ...
        '/login-form':'auth.loginForm@jinja:login'
...
exports.authenticatedRedirect= function (callback) {
    function action() {
        if(!this.request.session || this.request.session.user==null) {
            this.redirect('/login');
        }else
...
function save_user_prv(context, username,user,onSuccess) {
    ...
    v.expected=function(onSuccess){
        if(!user.hasOwnProperty("email"))
            v.addError("email","Campo richiesto");
        else
            onSuccess();
    };
...
function doLogin(params, self,onSuccess) {
...
exports.login = function () {
    var user = this.request.body.params;
    var self=this;
    doLogin(user, this,function(){
        self.renderer({
            status: "ok",
            user: user
        });
